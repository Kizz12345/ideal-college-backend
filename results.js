 ...
 *
 * Continuous Assessment components: 1st CA (10) + Note (5) + Assignment (5) + Mid Term Test (20) = 40
 * 1st C.A. (report sheet) = 1st CA (10) + Note (5) + Assignment (5)               = 20
 * 2nd C.A. (report sheet) = Mid Term Test (20)                                     = 20
 * Exam                                                                             = 60
 * TOTAL    = 1st C.A. + 2nd C.A. + Exam                                            = 100
 *
 * "Mid Term Score" (formerly "Test") subtab shows only the CA total (1st CA + 2nd CA)
 * per student per subject — it's a view, not an entry field.
 *
...
function computeSubjectEntry(s, teacher, existing = {}) {
  ...
  const firstCA = ca1 + note + assignment;   // max 20
  const secondCA = midTermTest;               // max 20
...
function getExistingSubject(studentId, session, term, subjectName) {
...
// mode: 'ca' (classwork/assignment/practical/quiz/test/project) or 'exam' (exam only) —
// whichever half isn't included in this request keeps its previously saved value.
router.post("/scores", authMiddleware, requireRole(...CAN_UPLOAD_SCORES), (req, res) => {
  ...
  logActivity(req.user.id, "scores_uploaded", { subject, session, term, mode, count: scores.length });
  res.json({ success: true, message: `Saved ${subject} scores for ${scores.length} student(s), signed ${initialsFor(teacher)}` });
});

// GET /api/results/class/:classGrade/:session/:term/:subject/export.csv?mode=ca|exam
router.get("/class/:classGrade/:session/:term/:subject/export.csv", authMiddleware, requireRole(...CAN_UPLOAD_SCORES), (req, res) => {
  ...
  const mode = req.query.mode === "exam" ? "exam" : "ca";
  const students = getStudentsInClass(classGrade).filter((s) => s.status === "active");

  const header = mode === "exam"
    ? "StudentID,Name,Subject,Exam(60)"
    : "StudentID,Name,Subject,1st CA(10),Note(5),Assignment(5),Mid Term Test(20)";
  const rows = [header];
  ...
  students.forEach((s) => {
    ...
    const line = mode === "exam"
      ? [s.id, `"${s.firstName} ${s.lastName}"`, subject, existing.exam || 0]
      : [s.id, `"${s.firstName} ${s.lastName}"`, subject, existing.ca1 || 0, existing.note || 0, existing.assignment || 0, existing.midTermTest || 0];
    rows.push(line.join(","));
  });

  res.setHeader("Content-Type", "text/csv");
  res.setHeader("Content-Disposition", `attachment; filename="${classGrade}_${subject}_${term}_${mode}.csv"`.replace(/\s+/g, "_"));
  res.send(rows.join("\n"));
});

// POST /api/results/import-csv  { subject, session, term, mode: 'ca'|'exam', csv }
router.post("/import-csv", authMiddleware, requireRole(...CAN_UPLOAD_SCORES), (req, res) => {
  ...
  lines.forEach((line) => {
    ...
    const existing = getExistingSubject(studentId, session, term, subject);

    let scoreInput;
    if (mode === "exam") {
      const [, , , exam] = cols;
      scoreInput = { name: subject, exam };
    } else {
      const [, , , ca1, note, assignment, midTermTest] = cols;
  ...
  logActivity(req.user.id, "scores_imported", { subject, session, term, mode, count });
...
router.get("/:studentId/:session/:term/report", authMiddleware, (req, res) => {
  ...
  const termSettings = db.prepare("SELECT currentTerm, resumptionDate, vacationDate FROM terms WHERE id = 1").get() || {};
  ...
  res.json({
    ...
    report: {
      ...
      endOfTerm: termSettings.vacationDate || null,
      resumptionDate: termSettings.resumptionDate || null
...
// Class teacher's comment: editable ONLY by the class teacher.
// Principal's comment: editable ONLY by proprietor/principal/vice principal.
router.patch("/:studentId/:session/:term/remarks", authMiddleware, requireRole(...LEADERSHIP_ROLES, "class_teacher"), (req, res) => {
  ...
  const isClassTeacher = req.user.role === "class_teacher";
  ...
  db.prepare(`UPDATE results SET teacherRemark = ?, principalRemark = ? WHERE studentId = ? AND session = ? AND term = ?`).run(
    isClassTeacher ? (teacherRemark ?? row.teacherRemark) : row.teacherRemark,
